  ...
  return new (class canvasParticles {
    ...
    update = function () {
        ...
        for (let i = 0; i < this.len; i++) {
          for (let j = i + 1; j < this.len; j++) {
            ...
            if (dist < this.options.particles.connectDist / 2) {
              ...
              const grav =
                (1 / Math.max(dist, 10)) ** 1.8 * this.options.particles.connectDist * this.options.gravity.repulsive
              ...
              const grav =
                (1 / Math.max(dist, 10)) ** 1.8 * this.options.particles.connectDist * this.options.gravity.pulling
              const gravX = Math.cos(angle) * grav
              const gravY = Math.sin(angle) * grav
              pointA.velX += gravX
              pointA.velY += gravY
      ...
      for (let point of this.particles) {
        ...
        point.posX =
          (point.posX + point.velX + ((Math.sin(point.dir) * point.speed) % this.width) + this.width) % this.width
        point.posY =
          (point.posY + point.velY + ((Math.cos(point.dir) * point.speed) % this.height) + this.height) % this.height
